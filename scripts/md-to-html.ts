  ...
  return new Promise((resolve, reject) => {
    ...
    const request = protocol.get(url, { headers: { 'User-Agent': 'Mozilla/5.0' } }, (response) => {
      ...
      if (response.statusCode !== 200) {
        file.close();
        fs.unlinkSync(destPath);
        reject(new Error(`Failed to download remote asset: ${response.statusCode}`));
        return;
      }
...
async function resolveImagePath(imagePath: string, baseDir: string, tempDir: string): Promise<string> {
  if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
    ...
    if (!fs.existsSync(localPath)) {
      console.error(`[md-to-html] Downloading remote image: ${imagePath}`);
...
function convertMarkdownToHtml(markdown: string, imageCallback: (src: string, alt: string) => string): { html: string; totalBlocks: number } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('```')) {
      if (inCodeBlock) {
        // X Articles rich editor doesn't support raw <pre><code> well, render as blockquote
    ...
    if (headingMatch) {
      ...
      if (level === 1) continue; // Skip H1 (handled as title)
...
export async function parseMarkdown(
  ...
  // Extract title from options, frontmatter, or first H1
  ...
  // Map placeholders to block indices
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    // If no cover image specified, use the first image in content as cover
  ...
  // Remove cover placeholder from HTML if first image became cover
  let finalHtml = html;
  if (coverPlaceholder) {
...
function printUsage(): never {
  console.log(`Convert Markdown documents into rich HTML formatted for X (Twitter) Articles

Usage:
  bun scripts/md-to-html.ts <markdown_file> [options]

Options:
  --title <title>       Override title from markdown frontmatter
  --cover <image>       Override cover image path
  --output <json|html>  Output format (default: json)
  --html-only           Output raw HTML string to stdout
  --save-html <path>    Write generated HTML to specific file
...
Examples:
  ...
  bun scripts/md-to-html.ts article.md --html-only > /tmp/article.html
  bun scripts/md-to-html.ts article.md --save-html /tmp/article.html
`);
  process.exit(0);
}
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(markdownPath)) {
    console.error(`Error: File not found at path: ${markdownPath}`);
    process.exit(1);
  }
  ...
  if (saveHtmlPath) {
    ...
    console.error(`[md-to-html] HTML written to: ${saveHtmlPath}`);
  }
