...
function printUsage(exitCode = 0): never {
  console.log(`Copy image or HTML content to system clipboard

Supports:
  - Image files (JPG, PNG, GIF, WebP) - copies raw image data
  - HTML content - copies as rich formatted text for pasting into editors

Usage:
  # Copy image to clipboard
  bun scripts/copy-to-clipboard.ts image /path/to/image.png

  # Copy HTML string to clipboard
  bun scripts/copy-to-clipboard.ts html "<p>Hello <strong>World</strong></p>"

  # Copy HTML from file
  bun scripts/copy-to-clipboard.ts html --file /path/to/content.html
`);
